-- Supabase replica of the Google Sheets attendance model.
-- Browser roles receive no direct table access; the server-side attendance
-- gateway uses the Supabase service role inside an Edge Function.
...
create table if not exists public.attendance_settings (
  ...
  location_name text not null default 'Abuja',
...
create index if not exists attendance_records_date_idx
  on public.attendance_records (attendance_date_key desc, id desc);

create index if not exists attendance_records_status_date_idx
  on public.attendance_records (status, attendance_date_key desc);

create index if not exists attendance_records_service_date_idx
  on public.attendance_records (service, attendance_date_key desc);

create index if not exists attendance_records_member_name_idx
  on public.attendance_records (lower(member_name));
