...
export function MemberLoginForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/member/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, email, password, code, rememberMe }) });
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    {step === "setup" && <>
      <div className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm text-cyan-50"><MailCheck className="mr-2 inline h-4 w-4" />A six-digit verification code was sent to your email.</div>
      <div className="space-y-2"><Label htmlFor="verification-code" className="font-semibold text-white">Verification code</Label><Input id="verification-code" inputMode="numeric" autoComplete="one-time-code" required pattern="[0-9]{6}" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} className="auth-input h-12 border-white/30 tracking-[0.35em]" /></div>
    ...
    {step === "email" && <p className="text-center text-xs font-medium leading-5 text-white/80">Enter your registered team email. First-time users will verify their email and create a private password—there is no shared password.</p>}
  </form>;
}
