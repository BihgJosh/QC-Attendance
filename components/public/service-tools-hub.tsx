...
const tools = [
  {
    ...
    tone: "amber",
    details: ["Typed member name with Team Data suggestions", "Automatically selected service date", "Uniform, behaviour and duty categories", "Observation, response and immediate action", "Automatic reporter attribution", "HOD-only review and resolution workflow"],
      ...
      <header className="safe-top-nav fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-6 sm:pt-5">
        ...
        {menuOpen && (
          <div id="service-tools-mobile-menu" className="mx-auto mt-2 max-w-7xl rounded-2xl border border-cyan-200/10 bg-[linear-gradient(145deg,rgba(2,12,32,0.98),rgba(74,20,96,0.96))] p-2 text-white shadow-2xl md:hidden">
            ...
            <Link href="/member/profile" onClick={() => setMenuOpen(false)} className="flex items-center justify-between rounded-xl px-4 py-3 text-sm font-medium text-white/80 hover:bg-white/10">My profile <UserRound className="h-4 w-4" /></Link>
