...
export function MemberDefaultReport({ canReview }: { canReview: boolean }) {
  ...
  const [category, setCategory] = useState<keyof typeof OPTIONS>("Uniform"), [requestId, setRequestId] = useState(() => crypto.randomUUID());
  ...
  const visibleReports = useMemo(() => { const query = search.trim().toLowerCase(); return query ? reports.filter((report) => `${report.member_name} ${report.category} ${report.default_type} ${report.status}`.toLowerCase().includes(query)) : reports; }, [reports, search]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    payload.requestId = requestId; payload.category = category; payload.memberInformed = form.get("memberInformed") === "yes"; payload.isRepeat = form.get("isRepeat") === "yes";
    try { const response = await fetch("/api/member-defaults", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); toast.success("Member default report filed."); formElement.reset(); setCategory("Uniform"); setRequestId(crypto.randomUUID()); if (canReview) await load(); }
  ...
  return <div className="p-4 sm:p-7">
    ...
    {loading ? <div className="grid min-h-56 place-items-center"><Loader2 className="h-7 w-7 animate-spin text-cyan-600" /></div> : <>
      <form onSubmit={submit} className="mt-7 grid gap-5 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 sm:grid-cols-2 sm:p-6">
        <Field label="Service date" htmlFor="default-service-date"><Input required id="default-service-date" name="serviceDate" type="date" className={inputClass} /></Field>
        <Field label="Service" htmlFor="default-service"><Input required id="default-service" name="service" placeholder="e.g. Sunday 1st Service" maxLength={80} className={inputClass} /></Field>
        <Field label="Member" htmlFor="default-member"><select required id="default-member" name="memberEmail" className="h-12 w-full rounded-[10px] border border-slate-300 bg-white px-4 text-base text-slate-950"><option value="">Select from Team Data</option>{members.map((member) => <option key={member.email} value={member.email}>{member.name}</option>)}</select></Field>
        <Field label="Category" htmlFor="default-category"><select id="default-category" value={category} onChange={(event) => setCategory(event.target.value as keyof typeof OPTIONS)} className="h-12 w-full rounded-[10px] border border-slate-300 bg-white px-4 text-base text-slate-950">{Object.keys(OPTIONS).map((item) => <option key={item}>{item}</option>)}</select></Field>
        <Field label="Location or duty post" htmlFor="default-location"><Input required id="default-location" name="location" maxLength={160} className={inputClass} /></Field>
        ...
        <Field label="Specific default" htmlFor="default-type"><select required id="default-type" name="defaultType" className="h-12 w-full rounded-[10px] border border-slate-300 bg-white px-4 text-base text-slate-950">{OPTIONS[category].map((item) => <option key={item}>{item}</option>)}</select></Field>
