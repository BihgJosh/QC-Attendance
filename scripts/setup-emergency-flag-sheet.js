...
async function main() {
  ...
  if (!title) throw new Error("The selected Emergency Flag tab was not found.");
  const escapedTitle = `'${title.replace(/'/g, "''")}'`;
  const result = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${escapedTitle}!A1:H1` });
  const current = (result.data.values?.[0] || []).map((value) => String(value).trim());
  if (!current.some(Boolean)) {
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: `${escapedTitle}!A1:H1`,
      valueInputOption: "RAW",
      requestBody: { values: [HEADERS] },
    });
    console.log(`Initialized "${title}" with ${HEADERS.length} Emergency Flag columns.`);
    return;
  }
  if (HEADERS.some((header, index) => current[index] !== header)) {
    throw new Error(`"${title}" has incompatible headers. No changes were made.`);
  }
  console.log(`Verified "${title}" with all ${HEADERS.length} Emergency Flag columns.`);
}
