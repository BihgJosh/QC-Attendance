...
PROFILE = {"profile": {"email": "mobile.test@example.com", "firstName": "Mobile", "middleName": "", "lastName": "Tester", "phone": "", "birthMonth": None, "birthDay": None, "qcJoinDate": "2024-01-15", "avatarUrl": None, "role": "general_user", "profileComplete": True}}
...
def upload_test(browser, mode):
    ...
    page.route("**/api/member/profile/image/stage", lambda route: route.fulfill(status=200, content_type="application/json", body=json.dumps({"signedUrl": f"{BASE_URL}/mock-upload/{mode}", "bucket": "member-profile-photo-staging", "objectPath": "test/source.jpg"})))
    ...
    def upload_route(route, request):
        if mode == "failure":
            route.fulfill(status=403, content_type="application/json", body='{"message":"signed upload token rejected"}')
            return
    ...
    join_date = page.get_by_label("When did you join Quality Control?")
    assert join_date.is_visible()
    assert join_date.get_attribute("required") is not None
    assert join_date.input_value() == "2024-01-15"
