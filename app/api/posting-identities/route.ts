...
export async function GET() {
  ...
  try {
    const [teamMember, teamMembers] = await Promise.all([getTeamMemberByEmail(session.email), listTeamMembers()]);
    ...
    const references = [...indexed.entries()].slice(0, 100).map(([key, member]) => ({ key, reference: { ...member, email: member.email || uniquelyMatchedEmail(member.name, teamMembers) } }));
    const resolved = await identityMap(session.token, references.map((item) => item.reference));
